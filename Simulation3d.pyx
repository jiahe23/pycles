...
class Simulation3d:
    ...
    def initialize(self, namelist):
        ...
        self.DV.add_variables('wBudget_TSin', 'm/s', r'wts1_in', 'w ts1 in', 'sym', self.Pa)
        # self.DV.add_variables('wBudget_wTS1_out', 'm/s', r'wts1_out', 'w ts1 out', 'sym', self.Pa)
        # self.DV.add_variables('wBudget_wTS2_in', 'm/s', r'wts2_in', 'w ts2 in', 'sym', self.Pa)
        # self.DV.add_variables('wBudget_TSout', 'm/s', r'wts2_out', 'w ts2 out', 'sym', self.Pa)
    ...
    def run(self):
        ...
        self.Ra.initialize_profiles(self.Gr, self.RS, self.DV, self.StatsIO,self.Pa)
...
#                self.Tr.update_cleanup(self.Gr, self.RS, PV_, DV_, self.Pa, self.TS)
    ...
    def io(self):
        ...
        if self.TS.t > 0 and self.TS.rk_step == self.TS.n_rk_steps - 1:
            ...
            self.Pa.root_print('sum of above two = '+str(self.StatsIO.last_output_time+self.StatsIO.frequency))
